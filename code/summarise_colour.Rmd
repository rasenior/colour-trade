...
# Repeat for traded species only
community_colour_trade <-
  # Apply function to every row
  apply(occ, MARGIN = 1, FUN = function(x){
    tryCatch(
      {summarise_cell(range_dat = x,
                      colour_dat = filter(spp_colour_sub, SiTDB_trade_interest == "evidence"))},
      error = function(x){
        message(x)
        return(NA)
      })})
...
saveRDS(community_colour_trade, file = "data/sorted/summarise-colour/community_colour_trade.Rds")
```
...
community_colour_trade <- readRDS("data/sorted/summarise-colour/community_colour_trade.Rds")
...
community_colour <-
  left_join(community_colour, colour_diversity, 
            by = c("longitude", "latitude", "patch"))
community_colour_trade <-
  left_join(community_colour_trade, colour_diversity, 
            by = c("longitude", "latitude", "patch"))
...
community_colour_trade <-
  left_join(community_colour_trade, select(envdat, -coords), 
            by = c("longitude", "latitude"))
